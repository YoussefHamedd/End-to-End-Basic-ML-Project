...
prediction_duration = Histogram(
    "model_prediction_duration_seconds", "Time spent processing prediction"
)

prediction_score_gauge = Gauge(
    "model_prediction_score", "Latest prediction score value"
)

error_counter = Counter("model_errors_total", "Total number of prediction errors")

feature_drift_gauge = Gauge(
    "feature_drift_score", "Data drift score for input features", ["feature"]
)
...
class ModelMonitor:
    ...
    def track_prediction(self, input_data, prediction, duration):
        ...
        self.predictions_history.append(
            {"input": input_data, "prediction": prediction, "timestamp": time.time()}
        )
